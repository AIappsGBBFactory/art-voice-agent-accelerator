...
def _normalize_responses_stream(raw_stream: Any):
    """Yield ChatCompletionChunk-shaped events from a Responses API stream.

    A thin generator function (not an adapter class, per design guidance)
    so the unchanged chat.completions consumption loop in
    ``_streaming_completion`` can drive both endpoints.
    """
...
class CascadeOrchestratorAdapter:
            ...
            try:
                ...
                def _streaming_completion():
                    ...
                    try:
                        ...
                        endpoint_name = "responses" if endpoint_choice == "responses" else "chat.completions"
                        ...
                        with tracer.start_as_current_span(
                            ...
                            attributes={
                                ...
                                "gen_ai.endpoint_type": "responses" if endpoint_choice == "responses" else "chat",
                            },
                        ) as openai_span:
                            ttft_tracker["request_start"] = time.perf_counter()
                            if endpoint_choice == "responses":
                                # Normalizer yields ChatCompletionChunk-shaped
                                # events so the loop below is unchanged for
                                # both endpoints.
                                stream = _normalize_responses_stream(
                                    request_client.responses.create(**api_params)
                                )
                            else:
                                stream = request_client.chat.completions.create(**api_params)
    ...
    def _prepare_responses_streaming_params(
        ...
        reasoning_effort = getattr(model_config, "reasoning_effort", None) if model_config else None
        include_reasoning = getattr(model_config, "include_reasoning", False) if model_config else False
        ...
        logger.debug(
            ...
            model_name, no_custom_temp, bool(reasoning_effort), include_reasoning, bool(verbosity),
        )
    ...
    def _extract_error_details(self, exception: Exception) -> str:
        ...
        if isinstance(exception, UnsupportedModelOptionError):
            return json.dumps({
                ...
                "details": (
                    "min_p and typical_p have no equivalent parameter on "
                    "chat.completions.create or responses.create in the "
                    "installed openai package and should always be disabled "
                    "in Advanced Builder. include_reasoning (an available "
                    "reasoning SUMMARY, never raw chain-of-thought) is only "
                    "supported when this agent's endpoint_preference is "
                    "'responses' (via reasoning.summary); disable or label it "
                    "as responses-only for agents left on 'auto'/'chat'."
                ),
                "unsupported_options": exception.options,
            })
