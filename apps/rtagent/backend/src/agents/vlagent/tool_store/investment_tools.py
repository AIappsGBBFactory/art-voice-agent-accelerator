...
async def get_account_routing_info(args: GetAccountRoutingInfoArgs) -> Dict[str, Any]:
    ...
    try:
        ...
        routing = bank_profile.get("routing_number", "026009593")  # BofA routing default
...
async def calculate_tax_impact(args: CalculateTaxImpactArgs) -> Dict[str, Any]:
    ...
    try:
        ...
        # Calculate tax impact based on rollover type
        tax_scenarios = {
            "direct_rollover": {
            ...
            "indirect_rollover": {
                ...
                "penalty": 0,
                "total_taxes": 0,
                "net_amount": amount * 0.80,  # You receive 80%, need to deposit full 100%
                "timeline": "60 days to complete rollover or face taxes + penalty",
                "recommendation": "Not recommended - complicated and risky",
                "warning": f"You'll receive ${amount * 0.80:,.2f} (80% of ${amount:,.2f}) but must deposit the full ${amount:,.2f} within 60 days to avoid taxes. You need to come up with the missing ${amount * 0.20:,.2f} from other sources.",
                "steps": [
                    "Old plan sends you a check for 80% of balance",
                    "You have 60 days to deposit FULL amount into new account",
                    "If you miss deadline or deposit less, IRS treats it as withdrawal",
                    "Withheld 20% refunded when you file taxes next year"
                ]
            },
            "roth_conversion": {
                ...
                "tax_withholding": 0,
                "penalty": 0,
                "total_taxes": amount * 0.25,  # Estimate 25% tax bracket
                ...
                "steps": [
                    ...
                    f"Estimated tax bill: ${amount * 0.25:,.2f} (depends on your tax bracket)",
            ...
            "cash_out": {
                ...
                "tax_withholding": amount * 0.20,
                "penalty": amount * 0.10,  # 10% early withdrawal penalty if under 59½
                "total_taxes": amount * 0.35,  # 25% income tax + 10% penalty estimate
                "net_amount": amount * 0.65,
                "timeline": "Immediate",
                "recommendation": "Strongly discouraged - loses 30-40% to taxes and penalties",
                "warning": f"You'll receive only ~${amount * 0.65:,.2f} from your ${amount:,.2f} balance. You lose ${amount * 0.35:,.2f} to taxes and penalties.",
                "consequences": [
                    f"${amount * 0.25:,.2f} in income taxes (25% bracket estimate)",
                    f"${amount * 0.10:,.2f} early withdrawal penalty (10%)",
