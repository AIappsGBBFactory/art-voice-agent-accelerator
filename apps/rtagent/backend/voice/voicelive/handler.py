...
# ─────────────────────────────────────────────────────────────────────────────
from apps.rtagent.backend.voice.voicelive.settings import get_settings
from apps.rtagent.backend.voice.voicelive.agent_adapter import adapt_unified_agents
from apps.rtagent.backend.voice.voicelive.session_loader import load_user_profile_by_email
from apps.rtagent.backend.voice.voicelive.tool_helpers import (
	push_tool_start,
	push_tool_end,
)
			...
			try:
				...
				try:
					await self._emit_agent_inventory(
						agents=agents,
						start_agent=effective_start_agent,
						source="unified" if app_state and getattr(app_state, "unified_agents", None) else "legacy",
				...
				# Priority 2: Restore from MemoManager (previous session context)
					...
					if stored_profile:
						...
						system_vars["customer_intelligence"] = stored_profile.get("customer_intelligence", {})
						...
						if system_vars.get("client_id"):
							logger.info("🔄 Restored partial context from memory | client_id=%s", system_vars.get("client_id"))
