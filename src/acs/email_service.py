    ...
    def send_email_background(
        ...
        def _send_email_background_task():
            try:
                ...
                if result.get("success"):
                    logger.info("📧 Background email sent successfully: %s", result.get("message_id"))
                else:
        ...
        try:
            email_thread = threading.Thread(
                target=_send_email_background_task,
                daemon=True
            )
            email_thread.start()
...
async def send_email(email_address: str, subject: str, plain_text_body: str, html_body: Optional[str] = None) -> Dict[str, Any]:
...
def send_email_background(
    ...
    email_service.send_email_background(email_address, subject, plain_text_body, html_body, callback)
