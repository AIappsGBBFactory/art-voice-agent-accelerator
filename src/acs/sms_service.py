...
class SmsService:
    ...
    def is_configured(self) -> bool:
        ...
        return (
            AZURE_SMS_AVAILABLE and 
            bool(self.connection_string) and 
            bool(self.from_phone_number)
        )
    
    async def send_sms(
        self, 
        to_phone_numbers: Union[str, List[str]], 
        message: str, 
        ...
        try:
            ...
            for response in sms_responses:
                ...
                if response.successful:
                    ...
                    logger.info("📱 SMS sent successfully to %s, message ID: %s", response.to, response.message_id)
                else:
                    failed_messages.append(message_data)
                    logger.error("📱 SMS failed to %s: %s", response.to, response.error_message if hasattr(response, 'error_message') else 'Unknown error')
    ...
    def send_sms_background(
        ...
        def _send_sms_background_task():
            try:
                ...
                if result.get("success"):
                    logger.info("📱 Background SMS sent successfully: %d messages", result.get("sent_count", 0))
                else:
        ...
        try:
            sms_thread = threading.Thread(
                target=_send_sms_background_task,
                daemon=True
            )
            sms_thread.start()
...
def send_sms_background(
    ...
    sms_service.send_sms_background(to_phone_numbers, message, enable_delivery_report, tag, callback)
