...
class TelemetryConfig:
    ...
    def from_env(cls) -> "TelemetryConfig":
        ...
        return cls(
            ...
            enable_openai_instrumentation=_bool_env("ENABLE_OPENAI_INSTRUMENTATION", True),
...
NOISY_SPAN_PATTERNS: List[Pattern[str]] = [
    ...
    # VoiceLive high-frequency events (audio streaming)
...
def _setup_openai_instrumentation(tracer_provider=None) -> bool:
    """Enable OpenAI client instrumentation for GenAI semantic conventions."""
    global _openai_instrumented
    
    if _openai_instrumented:
        return True
    
    # Check early-init module
    try:
    ...
    try:
        ...
        if "openai" in sys.modules:
            logger.warning("OpenAI imported before instrumentation - spans may not work correctly")
        ...
        _openai_instrumented = True
        logger.info(f"OpenAI instrumentation enabled (content_recording={content_recording})")
        return True
        ...
        logger.warning(f"Failed to enable OpenAI instrumentation: {e}")
        return False


def is_openai_instrumented() -> bool:
    """Check if OpenAI instrumentation is enabled."""
    return _openai_instrumented
...
def setup_azure_monitor(logger_name: str = None, config: TelemetryConfig = None) -> bool:
    ...
    # Also check if OpenTelemetry SDK TracerProvider is already set (e.g., by another module)
    try:
        from opentelemetry import trace as otel_trace
        ...
        if isinstance(current_provider, SDKTracerProvider):
            logger.warning(
                f"setup_azure_monitor: SDK TracerProvider already configured by another module - skipping to prevent duplicates"
            )
            _azure_monitor_configured = True  # Mark as configured since it's already done
            return True
    except Exception:
    ...
    try:
        ...
        # Setup OpenAI instrumentation - uses the global tracer provider set by configure_azure_monitor
        if config.enable_openai_instrumentation:
