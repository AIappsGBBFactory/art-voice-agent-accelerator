...
PII_ATTRIBUTE_NAMES = frozenset([
    # User identifiers
    "user.email", "user.phone", "user.name", "user.full_name",
    "customer.email", "customer.phone", "customer.name",
    "caller.phone", "caller.number", "caller.id",
    # Auth/session
    "auth.token", "access_token", "refresh_token", "api_key",
    "authorization", "x-api-key",
    # Network
    "http.client_ip", "client.address", "net.peer.ip",
    # ACS specific
    "acs.caller_id", "acs.phone_number", "phone.number",
])
...
class PIIScrubber:
    ...
    def scrub_dict(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            key: self.scrub_attribute_value(key, value)
